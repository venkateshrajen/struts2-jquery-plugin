...
(function($){_struts2_jquery={preBind:null,postBind:null,bind:function(el){if(el){var $el=$(el);el=$el[0];var attributes=el.attributes;var options={};for(var i=0;i<attributes.length;i++){options[attributes[i].name.toLowerCase()]=attributes[i].value;}
var tag=el.tagName.toLowerCase();if(typeof(_struts2_jquery.preBind)!="function"||_struts2_jquery.preBind($el)){var widget=$el.attr("widget")||tag;this[widget]($el,options);if(_struts2_jquery.postBind&&(typeof(_struts2_jquery.postBind)=="function")){return _struts2_jquery.postBind(el);}}}},widget:function(name,binder){if(name&&binder){this[name]=binder;}},presence:function($elem,options){if(options.disabled=='true'){$elem.attr("disabled",true);$elem.addClass("disabled");if(options.hidedisabled=="true"){$elem.hide();}}},base:function($elem,options){if(options.hidetopics){var topics=options.hidetopics.split(',');for(var i=0;i<topics.length;i++){$elem.subscribe(topics[i],'_struts2_jquery_hide',options);}}
...
this.presence($elem,options);},interactive:function($elem,options){if(options.enabletopics){var topics=options.enabletopics.split(',');for(var i=0;i<topics.length;i++){$elem.subscribe(topics[i],'_struts2_jquery_enable',options);}}
if(options.disabletopics){var topics=options.disabletopics.split(',');for(var i=0;i<topics.length;i++){$elem.subscribe(topics[i],'_struts2_jquery_disable',options);}}},container:function($elem,options,loadHandlerName){if(options.reloadtopics){var topics=options.reloadtopics.split(',');for(var i=0;i<topics.length;i++){var topic=topics[i];if(!$elem.isSubscribed(topic)){$elem.subscribe(topics[i],loadHandlerName,options);}}};$elem.bind('_struts2_jquery_trigger_fetch',function(event,fetchOptions,loadHandlerName){var containerTopic='_struts2_jquery_topic_load_'+fetchOptions.id;$elem.subscribe(containerTopic,loadHandlerName);$elem.publish(containerTopic,fetchOptions);if(fetchOptions.pollmillis&&fetchOptions.pollmillis>0){var self=$elem;setTimeout(function(){self.trigger('_struts2_jquery_trigger_fetch',[fetchOptions,loadHandlerName]);},fetchOptions.pollmillis);}
return false;});},input:function($elem,options,loadHandlerName){if(!options){return;}
if(options.reloadtopics){var topics=options.reloadtopics.split(',');for(var i=0;i<topics.length;i++){var topic=topics[i];if(!$elem.isSubscribed(topic)){$elem.subscribe(topics[i],loadHandlerName,options);}}}
...
if(options.onclicktopics){var topics=options.onclicktopics.split(',');for(var i=0;i<topics.length;i++){var topic=topics[i];$elem.createTopic(topic);$elem.bind('click',function(event){$target=$(this);if(!$target.disabled||$target.disabled!=true){var publishOptions=options||{};$.extend(publishOptions,event.data);publishOptions.disabled=false;$target.publish(topic,publishOptions,event);}});}}
...
options.src=href;$elem.publishOnEvent('click',actionTopic);},select:function($elem,options){var loadHandlerName='_struts2_jquery_container_load';this.base($elem,options);this.interactive($elem,options);this.container($elem,options,loadHandlerName);this.input($elem,options,loadHandlerName);if(options.src){$elem.trigger('_struts2_jquery_trigger_fetch',[options,loadHandlerName]);}},div:function($elem,options){var loadHandlerName='_struts2_jquery_container_load';this.base($elem,options);this.container($elem,options,loadHandlerName);if(options.draggable=='true'){var draggableOptionsStr=options.draggableoptions;var draggableOptions=window[draggableOptionsStr];if(!draggableOptions){draggableOptions=eval("( "+draggableOptionsStr+" )");}
$elem.draggable(draggableOptions);}
if(options.droppable=='true'){var droppableOptionsStr=options.droppableoptions;var droppableOptions=window[droppableOptionsStr];if(!droppableOptions&&droppableOptionsStr&&droppableOptionsStr.length>0){droppableOptions=eval("( "+droppableOptionsStr+" )");}else{sortableOptions={};}
$elem.droppable(droppableOptions);}
if(options.resizable=='true'){var resizableOptionsStr=options.resizableoptions;var resizableOptions=window[resizableOptionsStr];if(!resizableOptions&&resizableOptionsStr&&resizableOptionsStr.length>0){resizableOptions=eval("( "+resizableOptionsStr+" )");}else{sortableOptions={};}
$elem.resizable(resizableOptions);}
if(options.sortable=='true'){var sortableOptionsStr=options.sortableoptions;var sortableOptions=window[sortableOptionsStr];if(!sortableOptions&&sortableOptionsStr&&sortableOptionsStr.length>0){sortableOptions=eval("("+sortableOptionsStr+")");}else{sortableOptions={};}
var $div=$elem;if(options.onsortableupdatetopics){var topics=options.onsortableupdatetopics.split(',');sortableOptions.update=function(event,ui){for(var i=0;i<topics.length;i++){$div.publish(topics[i],ui,event);}};}
if(options.onsortablestarttopics){var topics=options.onsortablestarttopics.split(',');sortableOptions.start=function(event,ui){for(var i=0;i<topics.length;i++){$div.publish(topics[i],ui,event);}};}
if(options.onsortablesorttopics){var topics=options.onsortablesorttopics.split(',');sortableOptions.sort=function(event,ui){for(var i=0;i<topics.length;i++){$div.publish(topics[i],ui,event);}};}
if(options.onsortablestoptopics){var topics=options.onsortablestoptopics.split(',');sortableOptions.stop=function(event,ui){for(var i=0;i<topics.length;i++){$div.publish(topics[i],ui,event);}};}
if(options.onsortablereceivetopics){var topics=options.onsortablereceivetopics.split(',');sortableOptions.receive=function(event,ui){for(var i=0;i<topics.length;i++){$div.publish(topics[i],ui,event);}};}
if(options.onsortableremovetopics){var topics=options.onsortableremovetopics.split(',');sortableOptions.remove=function(event,ui){for(var i=0;i<topics.length;i++){$div.publish(topics[i],ui,event);}};}
$elem.sortable(sortableOptions);}
if(options.src){$elem.trigger('_struts2_jquery_trigger_fetch',[options,loadHandlerName]);}},form:function($elem,options){var submitHandlerName='_struts2_jquery_form_submit';var containerLoadHandlerName='_struts2_jquery_container_load';this.base($elem,options);if(options.onsubmittopics){var topics=options.onsubmittopics.split(',');for(var i=0;i<topics.length;i++){$elem.publishOnEvent('submit',topics[i]);}}
var formTopic='_struts2_jquery_form_topic_'+options.id;var action=options.action;if(action==null||action==""){options.action="#";}
options.src=options.action;options.formids=options.id;var targetId=options.targetid;if(options.submittopics){var topics=options.submittopics.split(',');for(var i=0;i<topics.length;i++){if(targetId){if('#tab'==targetId){$elem.closest('.ui-tabs-panel').subscribe(topics[i],containerLoadHandlerName,options);}else{$('#'+targetId).subscribe(topics[i],containerLoadHandlerName,options);}}else{$elem.subscribe(topics[i],submitHandlerName,options);}}}
if(targetId){$(function(){if('#tab'==targetId){$elem.closest('.ui-tabs-panel').subscribe(formTopic,containerLoadHandlerName,options);}else{$('#'+targetId).subscribe(formTopic,containerLoadHandlerName,options);}});}else{$elem.subscribe(formTopic,submitHandlerName,options);}
$elem.submit(function(event){event.preventDefault();$elem.publish(formTopic,event.data,event);});},a:function($elem,options){var linkLoadHandlerName='_struts2_jquery_action_request';var containerLoadHandlerName='_struts2_jquery_container_load';this.base($elem,options);this.interactive($elem,options);this.action($elem,options,containerLoadHandlerName,linkLoadHandlerName);},button:function($elem,options){var linkLoadHandlerName='_struts2_jquery_action_request';var containerLoadHandlerName='_struts2_jquery_container_load';this.base($elem,options);this.interactive($elem,options);this.action($elem,options,containerLoadHandlerName,linkLoadHandlerName);$elem.removeAttr('name');},dialog:function($elem,options){if(options.hidetopics){var topics=options.hidetopics.split(',');for(var i=0;i<topics.length;i++){$elem.subscribe(topics[i],'_struts2_jquery_dialog_close',options);}}
...
var parameters={};parameters.autoOpen=false;parameters.modal=eval(options.modal?options.modal:false);parameters.resizable=eval(options.resizable?options.resizable:true);parameters.sortable=eval(options.sortable?options.sortable:true);parameters.draggable=eval(options.draggable?options.draggable:true);if(options.height){parameters.height=eval(options.height);}
...
var $dialog=$elem;$dialog.data('buttonTopics',{});var buttons=options.buttons.split(',');for(var i=0;i<buttons.length;i++){var button=buttons[i];var topic=buttontopics[i];if(buttontopics.length>=i+1){$dialog.data('buttonTopics')[button]=topic;parameters.buttons[button]=function(event){$elem.publish($dialog.data('buttonTopics')[$(event.target).text()],$dialog,event);};}else{parameters.buttons[button]=function(event){};}}}
$elem.css("display","none");if(options.src){var loadHandlerName='_struts2_jquery_container_load';this.container($elem,options,loadHandlerName);$elem.bind('dialogopen',function(event,ui){$elem.trigger('_struts2_jquery_trigger_fetch',[options,loadHandlerName]);});}
...
$.extend(options,userOptions);options.disabled=[];$("ul > *:not(li)",$elem).appendTo($elem);var $tabs=$elem.tabs(options);$("a",$tabs).each(function(tabIndex,el){$tab=$(el);if($tab.attr("isdisabled")=='true'){$tabs.tabs('disable',tabIndex);}
...
var onBlurTopics=$tab.attr("onblurtopics");if(onBlurTopics){var topics=onBlurTopics.split(',');for(var i=0;i<topics.length;i++){$tab.publishOnEvent('blur',topics[i]);}}});},accordion:function($elem,options){if(!options){options={}};options.cache=options.iscache||false;var userOptionsStr=options.options;var userOptions=window[userOptionsStr];if(!userOptions){userOptions=eval("( "+userOptionsStr+" )");}
$.extend(options,userOptions);options.header='._struts2_jquery_class_accordionitem_header';options.autoHeight=options.autoheight;options.fillSpace=options.fillspace;options.clearStyle=options.clearstyle;var $accordion=$elem.accordion(options);if(options.disabled){$accordion.accordion('disable');}
if(options.removetopics){var topics=options.removetopics.split(',');for(var i=0;i<topics.length;i++){$elem.subscribe(topics[i],'_struts2_jquery_accordion_remove',options);}}
if(options.enabletopics){var topics=options.enabletopics.split(',');for(var i=0;i<topics.length;i++){$elem.subscribe(topics[i],'_struts2_jquery_accordion_enable',options);}}
if(options.disabletopics){var topics=options.disabletopics.split(',');for(var i=0;i<topics.length;i++){$elem.subscribe(topics[i],'_struts2_jquery_accordion_disable',options);}}
$("._struts2_jquery_class_accordionitem_body",$accordion).each(function(itemIndex,el){var $item=$(el);$item.index=itemIndex;if($item.attr("isactive")){$accordion.accordion('option','active',itemIndex);}
if($item.attr("disabled")=="true"){var $header=$item.prev();if($item.attr("hidedisabled")=="true"){$item.hide();$header.hide();}else{var clickEvents=$header.data("events").click;$header.unbind("click");$header.addClass("ui-state-disabled").bind("click.__struts2jquery",function(e){e.preventDefault();e.stopImmediatePropagation();});for(var i in clickEvents){$header.bind("click",clickEvents[i]);}}}
var hideTopics=$item.attr("hidetopics");if(hideTopics){var topics=hideTopics.split(',');for(var i=0;i<topics.length;i++){$item.subscribe(topics[i],'_struts2_jquery_accordion_hideItem',itemIndex);}}
var showTopics=$item.attr("showtopics");if(showTopics){var topics=showTopics.split(',');for(var i=0;i<topics.length;i++){$item.subscribe(topics[i],'_struts2_jquery_accordion_showItem',itemIndex);}}
if($item.attr("src")){var itemAttributes=$item[0].attributes;var itemOptions={};for(var i=0;i<itemAttributes.length;i++){itemOptions[itemAttributes[i].name.toLowerCase()]=itemAttributes[i].value;}
this.container($item,options,loadHandlerName);var loadHandlerName='_struts2_jquery_container_load';$item.trigger('_struts2_jquery_trigger_fetch',[options,loadHandlerName]);$accordion.bind('accordionchange',function(event,ui){if((ui.newHeader.next()[0]==$item[0])&&(!itemOptions.cache||!$item.data('loaded'))){$item.publish(loadTopic,itemOptions);}});if($item.hasClass(".ui-accordion-content-active")||!itemOptions.lazyload){$item.publish(loadTopic,itemOptions);}}});},textfield:function($elem,options){var loadHandlerName='_struts2_jquery_container_load';this.base($elem,options);this.interactive($elem,options);this.container($elem,options,loadHandlerName);this.input($elem,options,loadHandlerName);if(options.src){$elem.trigger('_struts2_jquery_trigger_fetch',[options,loadHandlerName]);}},textarea:function($elem,options){var loadHandlerName='_struts2_jquery_container_load';this.base($elem,options);this.interactive($elem,options);this.container($elem,options,loadHandlerName);this.input($elem,options,loadHandlerName);if(options.src){$elem.trigger('_struts2_jquery_trigger_fetch',[options,loadHandlerName]);}},datepicker:function($elem,options){var dpOptions={};dpOptions.altField="#"+$elem.attr("id")+"_hidden";dpOptions.altFormat="yy-mm-dd'T'00:00:00";dpOptions.buttonImageOnly=true;dpOptions.showOn="focus";if(options){if(options.hidetopics){var topics=options.hidetopics.split(',');for(var i=0;i<topics.length;i++){$elem.subscribe(topics[i],'_struts2_jquery_datepicker_hide',options);}}
...
if(isDisabled!=true&&isDisabled!='true'){var tagName=container[0].tagName.toLowerCase();var indicatorId=options.indicatorid;if(indicatorId){$('#'+indicatorId).show();}
...
var onSuccessTopics=options.onsuccesstopics;options.success=function(data,textStatus){container.data('loaded',true);if(indicatorId){$('#'+indicatorId).hide();}
if(tagName=='input'||tagName=='textarea'){container.val(data);}else if(tagName=='select'){container[0].length=0;if(typeof(data)=="object"||$.isArray(data)){var i=-1;if(options.headerkey&&options.headervalue){var option=document.createElement("option");option.value=options.headerkey;option.text=options.headervalue;if(options.value==options.headervalue){option.selected=true;}
container[0].options[++i]=option;}
if(options.emptyoption){container[0].options[++i]=document.createElement("option");}
...
container[0].options[++i]=option;}}}else{container.html(data);}
if(onSuccessTopics){var topics=onSuccessTopics.split(',');for(var i=0;i<topics.length;i++){container.publish(topics[i],container);}}
...
if(options.src){options.type="POST";options.url=options.src;if(tagName=='input'||tagName=='textarea'||tagName=='select'){options.dataType="json";}
...
var indicatorId=options.indicatorid;var onSuccessTopics=options.onsuccesstopics;options.success=function(data,textStatus){action.data('loaded',true);if(indicatorId){$('#'+indicatorId).hide();}
...
$.ajax(options);}}}});$.subscribeHandler('_struts2_jquery_datepicker_show',function(event,data){$(this).datepicker('show');});$.subscribeHandler('_struts2_jquery_datpicker_hide',function(event,data){$(this).datepicker('hide');});$.subscribeHandler('_struts2_jquery_datepicker_destroy',function(event,data){$(this).datepicker('destroy');});$.subscribeHandler('_struts2_jquery_datepicker_enable',function(event,data){$(this).datepicker('enable');});$.subscribeHandler('_struts2_jquery_datepicker_disable',function(event,data){$(this).datepicker('disable');});$.subscribeHandler('_struts2_jquery_dialog_open',function(event,data){$(this).dialog('open');});$.subscribeHandler('_struts2_jquery_dialog_close',function(event,data){$(this).dialog('close');});$.subscribeHandler('_struts2_jquery_dialog_destroy',function(event,data){$(this).dialog('destroy');});$.subscribeHandler('_struts2_jquery_dialog_enable',function(event,data){$(this).dialog('enable');});$.subscribeHandler('_struts2_jquery_dialog_disable',function(event,data){$(this).dialog('disable');});$.subscribeHandler('_struts2_jquery_reloadTab',function(event,data){$(this).closest("._struts2_jquery_class_tabbedpane").tabs('load',event.data);});$.subscribeHandler('_struts2_jquery_selectTab',function(event,data){$(this).closest("._struts2_jquery_class_tabbedpane").tabs('select',event.data);});$.subscribeHandler('_struts2_jquery_disableTab',function(event,data){$(this).closest("._struts2_jquery_class_tabbedpane").tabs('disable',event.data);});$.subscribeHandler('_struts2_jquery_enableTab',function(event,data){$(this).closest("._struts2_jquery_class_tabbedpane").tabs('enable',event.data);});$.subscribeHandler('_struts2_jquery_removeTab',function(event,data){$(this).closest("._struts2_jquery_class_tabbedpane").tabs('remove',event.data);});$.subscribeHandler('_struts2_jquery_showTab',function(event,data){$(this).closest("._struts2_jquery_class_tabbedpane").tabs('show',event.data);});$.subscribeHandler('_struts2_jquery_hideTab',function(event,data){$(this).closest("._struts2_jquery_class_tabbedpane").tabs('remove',event.data);});$.subscribeHandler('_struts2_jquery_accordion_remove',function(event,data){$(this).accordion('destroy');});$.subscribeHandler('_struts2_jquery_accordion_enable',function(event,data){$(this).accordion('enable');});$.subscribeHandler('_struts2_jquery_accordion_disable',function(event,data){$(this).accordion('disable');});$.subscribeHandler('_struts2_jquery_accordion_hideItem',function(event,data){var accordion=$(".accordion");var items=$.map($(this).parent().find("dt"),function(a){return $(a).hasClass("ui-state-active");});var activeIndex=$.inArray(true,items);var $this=$(this);if(activeIndex==$this.index){$(this).prev().click();}});$.subscribeHandler('_struts2_jquery_accordion_showItem',function(event,data){var accordion=$(".accordion");var items=$.map($(this).parent().find("dt"),function(a){return $(a).hasClass("ui-state-active");});var activeIndex=$.inArray(true,items);var $this=$(this);var $this=$(this);if(activeIndex!=$this.index){$(this).prev().click();}});$.subscribeHandler('_struts2_jquery_form_submit',function(event,data){var form=$(event.target);form.submit();});})(jQuery);
...
(function($){var _subscribe_topics={};var _subscribe_handlers={};$.fn.extend({createTopic:function(topic){if(!_subscribe_topics[topic]){_subscribe_topics[topic]={};_subscribe_topics[topic].objects={};_subscribe_topics[topic].objects['__noId__']=[];}
...
return this;},publish:function(topic,data){this.createTopic(topic);for(i in _subscribe_topics[topic].objects){var object=_subscribe_topics[topic].objects[i];if($.isArray(object)&&object.length>0){for(j in object){object[j].trigger(topic,data);}}else{object.trigger(topic,data);}}
...
return $();},publish:function(topic,data){return $().publish(topic,data);},createTopic:function(topic){return $().createTopic(topic);},destroyTopic:function(topic){return $().destroyTopic(topic);}});_subscribe_handlers['reload']=function reload(event,data)
{$(this).load($(this).data("url"),data,$(this).data("oncomplete"));};})(jQuery);
...
return this;},publishOnEvent:function(event,topic,data){this.createTopic(topic);this.bind(event,data,function(e){$(this).publish(topic,e.data);});return this;}});$.extend({subscribe:function(topic,handler,data){return $().subscribe(topic,handler,data);},subscribeHandler:function(name,handler){if(name&&handler&&typeof(handler)=="function"){_subscribe_handlers[name]=handler;}

...
var actionTopic='_struts2_jquery_topic_'+options.id;var href=options.href;if(options.targets){$(function(){var topics=options.targets.split(',');for(var i=0;i<topics.length;i++){var target=topics[i];if('#tab'==target){$elem.closest('.ui-tabs-panel').subscribe(actionTopic,containerLoadHandlerName,options);}else{$('#'+target).subscribe(actionTopic,containerLoadHandlerName,options);}}});}else{if(href!=null||options.oncompletetopics||options.onsuccesstopics||options.onerrortopics){$elem.subscribe(actionTopic,linkLoadHandlerName,options);}}
...
var parameters={};parameters.autoOpen=false;parameters.modal=(options.modal?options.modal:false);parameters.resizable=(options.resizable?options.resizable:true);if(options.height){parameters.height=options.height;}
...
$elem.dialog(parameters);$(".ui-dialog-content",$elem).attr("id",$elem.attr("id"));$elem.removeAttr("id");},tabbedpane:function($elem,options){var $tabs=$elem.tabs({cache:(options.iscache||false),selected:options.selected});("a",$tabs).each(function(tabIndex,el){$tab=$(el);if($tab.attr("isdisabled")){$tabs.tabs('disable',tabIndex);}

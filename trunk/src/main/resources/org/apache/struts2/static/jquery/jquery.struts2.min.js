...
$elem.css("display","none");$elem.dialog(parameters);},tabbedpane:function($elem,options){var $tabs=$elem.tabs({cache:(options.iscache||false),selected:options.selected});("a",$tabs).each(function(tabIndex,el){$tab=$(el);if($tab.attr("isdisabled")){$tabs.tabs('disable',tabIndex);}
